...
| `PYTHONUTF8=1` | 사용자 환경변수 | 파이썬이 파일을 열고 결과를 찍을 때 쓰는 기본 인코딩을 UTF-8로 바꾼다. 이것이 없으면 한국어 윈도에서 cp949가 기본이라 클로드가 UTF-8로 쓴 결과가 깨진다. **이미 있는 값은 무엇이든 덮지 않는다.** 끄려면 이 값을 `0`으로 바꾼 뒤 창을 닫았다 연다. 지우면 다음 실행에서 다시 `1`이 서므로 끄는 방법이 못 된다. 기본값이 바뀌므로, 인코딩을 안 적고 cp949 파일을 읽거나 쓰던 기존 스크립트는 이 뒤로 깨질 수 있다 |
