...
| `extraKnownMarketplaces`·`enabledPlugins` | 플러그인 선언 추가 | CLI 설치가 막힌 PC에서도 첫 실행 때 들어오도록 남기는 선언이다 |
...
| 스킬 `document-formats`·`register-corp-certs` | 개인 스킬 폴더 | 앞은 문서 형식별 처리를, 뒤는 컨테이너 인증서를 다룬다 |
