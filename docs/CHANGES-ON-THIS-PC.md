...
| `pandas<3.0` | 표 데이터를 계산하고 정리한다 | 판을 3.0 미만으로 묶는다. 이 제약은 그대로 둔다 |
...
| `kw-doc-formats` | 사내 문서 형식 처리 스킬 `kw-doc-formats:document-formats`다. 이전 판은 이 스킬을 개인 스킬 폴더에 복사했는데, 이제 플러그인으로 온다. 고친 것이 이미 깔린 PC까지 닿게 하려고 설치기가 이 마켓플레이스의 자동 갱신을 켠다. 자동 갱신을 어디에 어떻게 켜는지는 아래 「고쳐지는 설정 파일」에 있다 |
...
| `PYTHONUTF8=1` | 사용자 환경변수 | **먼저 재고 필요할 때만 세운다.** 파이썬에게 기본 인코딩을 물어(`locale.getpreferredencoding`) 이미 UTF-8이면 건드리지 않고, cp949 같은 코드 페이지면 이 변수를 세워 UTF-8로 바꾼다. 재지 못하면 세우지 않고 경고한다. 이것이 없으면 클로드가 UTF-8로 쓴 결과가 깨진다. **이미 있는 값은 무엇이든 덮지 않는다.** 끄려면 이 값을 `0`으로 바꾼 뒤 창을 닫았다 연다. 지우면 다음 실행에서 다시 서므로 끄는 방법이 못 된다. 기본값이 바뀌므로, 인코딩을 안 적고 cp949 파일을 읽거나 쓰던 기존 스크립트는 이 뒤로 깨질 수 있다 |
